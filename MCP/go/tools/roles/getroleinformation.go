...
func GetroleinformationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/roles/%s%s", cfg.BaseURL, roleId, queryString)
...
func CreateGetroleinformationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_roles_roleId",
		mcp.WithDescription("Get role information"),
		mcp.WithString("roleId", mcp.Required(), mcp.Description("Role Id. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetroleinformationHandler(cfg),
	}
}
