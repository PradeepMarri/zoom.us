...
func RolemembersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["page_count"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("page_count=%v", val))
		}
...
func CreateRolemembersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_roles_roleId_members",
		mcp.WithDescription("List members in a role"),
		mcp.WithString("roleId", mcp.Required(), mcp.Description("The role ID")),
		mcp.WithString("page_count", mcp.Description("The number of pages returned for this request.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    RolemembersHandler(cfg),
	}
}
