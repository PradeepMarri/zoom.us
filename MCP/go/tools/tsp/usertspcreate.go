...
func UsertspcreateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/tsp%s", cfg.BaseURL, userId, queryString)
...
func CreateUsertspcreateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_users_userId_tsp",
		mcp.WithDescription("Add a user's TSP account"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UsertspcreateHandler(cfg),
	}
}
