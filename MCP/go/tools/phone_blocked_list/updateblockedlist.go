...
func UpdateblockedlistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdateblockedlistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_phone_blocked_list_blockedListId",
		mcp.WithDescription("Update a blocked list"),
		mcp.WithString("blockedListId", mcp.Required(), mcp.Description("Unique Identifier of the blocked list.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdateblockedlistHandler(cfg),
	}
}
