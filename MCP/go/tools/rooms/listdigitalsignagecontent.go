...
func ListdigitalsignagecontentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["folder_id"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("folder_id=%v", val))
		}
		...
		url := fmt.Sprintf("%s/rooms/digital_signage%s", cfg.BaseURL, queryString)
...
func CreateListdigitalsignagecontentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rooms_digital_signage",
		mcp.WithDescription("List digital signage contents"),
		mcp.WithString("type", mcp.Description("Specify the type of digital signane resource. The value can be one of the following:\n* `content`: Returns information about content files.\n* `folder`: Returns information about the folder where the content files are located.")),
		mcp.WithString("folder_id", mcp.Description("Unique identifier of the folder where the content is located. Provide this field if you would like to filter the response by contents that are only available in a specific folder.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListdigitalsignagecontentHandler(cfg),
	}
}
