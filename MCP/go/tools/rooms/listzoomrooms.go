...
func ListzoomroomsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["unassigned_rooms"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("unassigned_rooms=%v", val))
		}
		...
		if val, ok := args["location_id"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("location_id=%v", val))
		}
		...
		url := fmt.Sprintf("%s/rooms%s", cfg.BaseURL, queryString)
...
func CreateListzoomroomsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rooms",
		mcp.WithDescription("List Zoom Rooms"),
		mcp.WithString("status", mcp.Description("The status of the Zoom Room.")),
		mcp.WithString("type", mcp.Description("Type of the Zoom Rooms.")),
		mcp.WithBoolean("unassigned_rooms", mcp.Description("Use this query parameter with a value of `true` if you would like to see Zoom Rooms in your account that have not been assigned to anyone yet.")),
		...
		mcp.WithString("location_id", mcp.Description("Parent location ID of the Zoom Room.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListzoomroomsHandler(cfg),
	}
}
