...
func UpdatezrsettingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdatezrsettingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_rooms_roomId_settings",
		mcp.WithDescription("Update Zoom Room settings"),
		mcp.WithString("roomId", mcp.Required(), mcp.Description("Unique Identifier of the Zoom Room.")),
		mcp.WithString("setting_type", mcp.Description("The type of setting that you would like to update.<br> `alert`: Alert Settings applied on the Zoom Room.<br>\n`meeting`: Meeting settings of the Zoom Room.<br>\n`signage`: Digital signage settings applied on the Zoom Room.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdatezrsettingsHandler(cfg),
	}
}
