...
func ListzrlocationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["parent_location_id"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("parent_location_id=%v", val))
		}
...
func CreateListzrlocationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rooms_locations",
		mcp.WithDescription("List Zoom Room locations"),
		mcp.WithString("parent_location_id", mcp.Description("A unique identifier of the parent location. For instance, if a Zoom Room is located in Floor 1 of Building A, the location of Building A will be the parent location of Floor 1. Use this parameter to filter the response by a specific location hierarchy level.")),
		mcp.WithString("type", mcp.Description("Use this field to filter the response by the type of location. The value can be one of the following:\n`country`, `states`, `city`, `campus`, `building`, `floor`. ")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListzrlocationsHandler(cfg),
	}
}
