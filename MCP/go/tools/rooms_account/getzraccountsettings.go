...
func GetzraccountsettingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetzraccountsettingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rooms_account_settings",
		mcp.WithDescription("Get Zoom Room account settings"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetzraccountsettingsHandler(cfg),
	}
}
