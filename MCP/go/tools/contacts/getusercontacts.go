...
func GetusercontactsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/chat/users/me/contacts%s", cfg.BaseURL, queryString)
...
func CreateGetusercontactsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_chat_users_me_contacts",
		mcp.WithDescription("List user's contacts"),
		mcp.WithString("type", mcp.Description("The type of contact. The value can be one of the following:\n`company`: Contacts from the user's organization.\n`external`: External contacts. ")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetusercontactsHandler(cfg),
	}
}
