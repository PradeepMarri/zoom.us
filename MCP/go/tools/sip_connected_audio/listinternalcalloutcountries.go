...
func ListinternalcalloutcountriesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateListinternalcalloutcountriesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_accounts_accountId_sip_trunk_callout_countries",
		mcp.WithDescription("List internal call-out countries"),
		mcp.WithString("accountId", mcp.Required(), mcp.Description("Unique identifier of the account. To list Call-out enabled countries to a sub account, provide the account ID of the sub account in the `accountId` path parameter. To list Call-out enabled countries of a sub account, provide the account ID of the sub account in the `accountId` path parameter. To list Call-out enabled countries of a master account, provide `me` as the value of the `accountId` path parameter.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListinternalcalloutcountriesHandler(cfg),
	}
}
