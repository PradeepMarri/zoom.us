...
func AssignsiptrunksHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateAssignsiptrunksTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_accounts_accountId_sip_trunk_trunks",
		...
		mcp.WithArray("sip_trunks", mcp.Description("Input parameter: Array of one or more SIP Trunk objects. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    AssignsiptrunksHandler(cfg),
	}
}
