...
func AddinternalnumbersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/accounts/%s/sip_trunk/internal_numbers%s", cfg.BaseURL, accountId, queryString)
...
func CreateAddinternalnumbersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_accounts_accountId_sip_trunk_internal_numbers",
		mcp.WithDescription("Add internal numbers"),
		mcp.WithString("accountId", mcp.Required(), mcp.Description("Unique identifier of the account.<br>To add internal numbers to a sub account, provide the account ID of the sub account in the `accountId` path parameter. To add internal numbers to a master account, provide `me` as the value of the `accountId` path parameter.")),
		mcp.WithArray("internal_numbers", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    AddinternalnumbersHandler(cfg),
	}
}
