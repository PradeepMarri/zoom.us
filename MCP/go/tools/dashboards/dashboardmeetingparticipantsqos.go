...
func DashboardmeetingparticipantsqosHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/metrics/meetings/%s/participants/qos%s", cfg.BaseURL, meetingId, queryString)
...
func CreateDashboardmeetingparticipantsqosTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_metrics_meetings_meetingId_participants_qos",
		mcp.WithDescription("List meeting participants QoS"),
		...
		mcp.WithNumber("page_size", mcp.Description("The number of items returned per page.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DashboardmeetingparticipantsqosHandler(cfg),
	}
}
