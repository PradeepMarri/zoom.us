...
func DashboardmeetingparticipantsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/metrics/meetings/%s/participants%s", cfg.BaseURL, meetingId, queryString)
...
func CreateDashboardmeetingparticipantsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_metrics_meetings_meetingId_participants",
		mcp.WithDescription("List meeting participants"),
		...
		mcp.WithString("type", mcp.Description("The meeting types: <br>`past` - Past meetings.<br>`pastOne` - Past one user meetings.<br>`live` - Live meetings.")),
