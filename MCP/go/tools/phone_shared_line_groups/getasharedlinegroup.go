...
func GetasharedlinegroupHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetasharedlinegroupTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_phone_shared_line_groups_sharedLineGroupId",
		mcp.WithDescription("Get a shared line group"),
		mcp.WithString("sharedLineGroupId", mcp.Required(), mcp.Description("Unique Identifier of the Shared Line Group.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetasharedlinegroupHandler(cfg),
	}
}
