...
func DeleteasharedlinegroupHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleteasharedlinegroupTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_phone_shared_line_groups_sharedLineGroupId",
		mcp.WithDescription("Delete a shared line group"),
		mcp.WithString("sharedLineGroupId", mcp.Required(), mcp.Description("Unique Identifier of the shared line group that you would like to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteasharedlinegroupHandler(cfg),
	}
}
