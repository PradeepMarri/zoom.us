...
func DeletesipphoneHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletesipphoneTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_sip_phones_phoneId",
		mcp.WithDescription("Delete SIP phone"),
		mcp.WithString("phoneId", mcp.Required(), mcp.Description("Unique Identifier of the SIP Phone. It can be retrieved from the List SIP Phones API.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletesipphoneHandler(cfg),
	}
}
