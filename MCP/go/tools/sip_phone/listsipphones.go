...
func ListsipphonesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["search_key"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("search_key=%v", val))
		}
...
func CreateListsipphonesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_sip_phones",
		mcp.WithDescription("List SIP phones"),
		mcp.WithNumber("page_number", mcp.Description("\n**Deprecated** - This field has been deprecated and we will stop supporting it completely in a future release. Please use \"next_page_token\" for pagination instead of this field.\n\nThe page number of the current page in the returned records.")),
		mcp.WithString("search_key", mcp.Description("User name or email address of a user. If this parameter is provided, only the SIP phone system integration enabled for that specific user will be returned. Otherwise, all SIP phones on an account will be returned.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListsipphonesHandler(cfg),
	}
}
