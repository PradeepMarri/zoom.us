...
func MeetingHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateMeetingTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_meetings_meetingId",
		mcp.WithDescription("Get a meeting"),
		...
		mcp.WithString("occurrence_id", mcp.Description("Meeting Occurrence ID. Provide this field to view meeting details of a particular occurrence of the [recurring meeting](https://support.zoom.us/hc/en-us/articles/214973206-Scheduling-Recurring-Meetings).")),
		mcp.WithBoolean("show_previous_occurrences", mcp.Description("Set the value of this field to `true` if you would like to view meeting details of all previous occurrences of a [recurring meeting](https://support.zoom.us/hc/en-us/articles/214973206-Scheduling-Recurring-Meetings). ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    MeetingHandler(cfg),
	}
}
