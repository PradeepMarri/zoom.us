...
func MeetingregistrantstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/meetings/%s/registrants/status%s", cfg.BaseURL, meetingId, queryString)
...
func CreateMeetingregistrantstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_meetings_meetingId_registrants_status",
		mcp.WithDescription("Update registrant's status"),
		...
		mcp.WithArray("registrants", mcp.Description("Input parameter: List of registrants.")),
		mcp.WithString("action", mcp.Required(), mcp.Description("Input parameter: Registrant Status:<br>`approve` - Approve registrant.<br>`cancel` - Cancel previously approved registrant's registration.<br>`deny` - Deny registrant.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    MeetingregistrantstatusHandler(cfg),
	}
}
