...
func PastmeetingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/past_meetings/%s/instances%s", cfg.BaseURL, meetingId, queryString)
...
func CreatePastmeetingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_past_meetings_meetingId_instances",
		mcp.WithDescription("List ended meeting instances"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    PastmeetingsHandler(cfg),
	}
}
