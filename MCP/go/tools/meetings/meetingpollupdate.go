...
func MeetingpollupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/meetings/%s/polls/%s%s", cfg.BaseURL, meetingId, pollId, queryString)
...
func CreateMeetingpollupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_meetings_meetingId_polls_pollId",
		mcp.WithDescription("Update a meeting poll"),
		...
		mcp.WithString("title", mcp.Description("Input parameter: Title for the poll.")),
		mcp.WithArray("questions", mcp.Description("Input parameter: Array of Polls")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    MeetingpollupdateHandler(cfg),
	}
}
