...
func ImgroupcreateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/im/groups%s", cfg.BaseURL, queryString)
...
func CreateImgroupcreateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_im_groups",
		mcp.WithDescription("Create an IM directory group"),
		...
		mcp.WithString("type", mcp.Description("Input parameter: IM Group types:<br>`normal` - Only members can see automatically see the other members of this group. Other people can search for members within this group. <br>`shared` - Everyone under an account can see the group members automatically.<br>`restricted` - Nobody can see the group or search for members except the members in the group.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ImgroupcreateHandler(cfg),
	}
}
