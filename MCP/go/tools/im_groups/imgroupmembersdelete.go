...
func ImgroupmembersdeleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/im/groups/%s/members/%s%s", cfg.BaseURL, groupId, memberId, queryString)
...
func CreateImgroupmembersdeleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_im_groups_groupId_members_memberId",
		mcp.WithDescription("Delete an IM directory group member"),
		mcp.WithString("groupId", mcp.Required(), mcp.Description("The group ID.<br>\nCan be retrieved by calling [GET /groups](https://marketplace.zoom.us/docs/api-reference/zoom-api/groups/groups).")),
		mcp.WithString("memberId", mcp.Required(), mcp.Description("The member ID.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ImgroupmembersdeleteHandler(cfg),
	}
}
