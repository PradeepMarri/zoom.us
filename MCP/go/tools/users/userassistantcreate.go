...
func UserassistantcreateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/assistants%s", cfg.BaseURL, userId, queryString)
...
func CreateUserassistantcreateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_users_userId_assistants",
		mcp.WithDescription("Add assistants"),
		...
		mcp.WithArray("assistants", mcp.Description("Input parameter: List of User's assistants.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UserassistantcreateHandler(cfg),
	}
}
