...
func UserssotokendeleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUserssotokendeleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_users_userId_token",
		mcp.WithDescription("Revoke a user's SSO token"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UserssotokendeleteHandler(cfg),
	}
}
