...
func UseremailupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/email%s", cfg.BaseURL, userId, queryString)
...
func CreateUseremailupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_users_userId_email",
		mcp.WithDescription("Update a user's email"),
		...
		mcp.WithString("email", mcp.Required(), mcp.Description("Input parameter: User’s email. The length should be less than 128 characters.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UseremailupdateHandler(cfg),
	}
}
