...
func UserstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/status%s", cfg.BaseURL, userId, queryString)
...
func CreateUserstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_users_userId_status",
		mcp.WithDescription("Update user status"),
		...
		mcp.WithString("action", mcp.Required(), mcp.Description("Input parameter: The action types:<br>`activate` - Activate a deactivated user.<br>`deactivate` - Deactivate a user.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UserstatusHandler(cfg),
	}
}
