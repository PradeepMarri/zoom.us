...
func ListphonedevicesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/phone/devices%s", cfg.BaseURL, queryString)
...
func CreateListphonedevicesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_phone_devices",
		mcp.WithDescription("List devices"),
		mcp.WithString("type", mcp.Required(), mcp.Description("State of the device. The value should be either `assigned` to list devices that have been assigned to user(s) or `unassigned` to list devices that have not yet been assigned to any user in the Zoom account.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListphonedevicesHandler(cfg),
	}
}
