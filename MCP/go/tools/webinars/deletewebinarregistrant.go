...
func DeletewebinarregistrantHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletewebinarregistrantTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_webinars_webinarId_registrants_registrantId",
		mcp.WithDescription("Delete a webinar registrant"),
		mcp.WithNumber("webinarId", mcp.Required(), mcp.Description("The webinar ID.")),
		mcp.WithString("registrantId", mcp.Required(), mcp.Description("The registrant ID.")),
		mcp.WithString("occurrence_id", mcp.Description("The webinar occurence ID.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletewebinarregistrantHandler(cfg),
	}
}
