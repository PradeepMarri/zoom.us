...
func WebinarstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/webinars/%s/status%s", cfg.BaseURL, webinarId, queryString)
...
func CreateWebinarstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_webinars_webinarId_status",
		mcp.WithDescription("Update webinar status"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    WebinarstatusHandler(cfg),
	}
}
