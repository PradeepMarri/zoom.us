...
func PastwebinarsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/past_webinars/%s/instances%s", cfg.BaseURL, webinarId, queryString)
...
func CreatePastwebinarsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_past_webinars_webinarId_instances",
		mcp.WithDescription("List past webinar instances"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    PastwebinarsHandler(cfg),
	}
}
