...
func WebinarregistrantstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/webinars/%s/registrants/status%s", cfg.BaseURL, webinarId, queryString)
...
func CreateWebinarregistrantstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_webinars_webinarId_registrants_status",
		...
		mcp.WithString("action", mcp.Required(), mcp.Description("Input parameter: Used to approve a registrant, deny a registrant, or cancel a previously approved registrant. ")),
		mcp.WithArray("registrants", mcp.Description("Input parameter: List of registrants.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    WebinarregistrantstatusHandler(cfg),
	}
}
