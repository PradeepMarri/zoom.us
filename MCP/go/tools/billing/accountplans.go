...
func AccountplansHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateAccountplansTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_accounts_accountId_plans",
		mcp.WithDescription("Get plan Information"),
		mcp.WithString("accountId", mcp.Required(), mcp.Description("The account ID.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    AccountplansHandler(cfg),
	}
}
