...
func AccountplanaddoncreateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateAccountplanaddoncreateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_accounts_accountId_plans_addons",
		mcp.WithDescription("Subscribe additional plan"),
		mcp.WithString("accountId", mcp.Required(), mcp.Description("The account ID.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    AccountplanaddoncreateHandler(cfg),
	}
}
