...
func AccountbillingHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateAccountbillingTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_accounts_accountId_billing",
		mcp.WithDescription("Get billing information"),
		mcp.WithString("accountId", mcp.Required(), mcp.Description("The account ID.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    AccountbillingHandler(cfg),
	}
}
