...
func AccounttrusteddomainHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/accounts/%s/trusted_domains%s", cfg.BaseURL, accountId, queryString)
...
func CreateAccounttrusteddomainTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_accounts_accountId_trusted_domains",
		mcp.WithDescription("Get trusted domains"),
		mcp.WithString("accountId", mcp.Required(), mcp.Description("The account ID.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    AccounttrusteddomainHandler(cfg),
	}
}
