...
func AccountdisassociateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/accounts/%s%s", cfg.BaseURL, accountId, queryString)
...
func CreateAccountdisassociateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_accounts_accountId",
		mcp.WithDescription("Disassociate a sub account"),
		mcp.WithString("accountId", mcp.Required(), mcp.Description("The account ID.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    AccountdisassociateHandler(cfg),
	}
}
