...
func ListarchivedfilesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["query_data_type"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("query_data_type=%v", val))
		}
		...
		url := fmt.Sprintf("%s/archive_files%s", cfg.BaseURL, queryString)
...
func CreateListarchivedfilesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_archive_files",
		mcp.WithDescription("List archived files"),
		...
		mcp.WithString("from", mcp.Description("Start date for the query in \"yyyy-MM-dd'T'HH:mm:ss'Z'\" format. The duration for the query defined using the \"from\" and \"to\" parameters should not exceed 7 days as this API only provides a week's data at once.")),
		mcp.WithString("to", mcp.Description("End date for the query in \"yyyy-MM-dd'T'HH:mm:ss'Z'\" format. ")),
		mcp.WithString("query_data_type", mcp.Description("Query data type for `from` and `to`.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListarchivedfilesHandler(cfg),
	}
}
