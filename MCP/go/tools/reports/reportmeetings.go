...
func ReportmeetingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/report/users/%s/meetings%s", cfg.BaseURL, userId, queryString)
...
func CreateReportmeetingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_report_users_userId_meetings",
		mcp.WithDescription("Get meeting reports"),
		...
		mcp.WithString("type", mcp.Description("The meeting types: <br>`past` - Past meetings.<br>`pastOne` - Past one user meetings. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ReportmeetingsHandler(cfg),
	}
}
