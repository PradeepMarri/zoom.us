...
func ReporttelephoneHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/report/telephone%s", cfg.BaseURL, queryString)
...
func CreateReporttelephoneTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_report_telephone",
		mcp.WithDescription("Get telephone reports"),
		mcp.WithString("type", mcp.Description("Audio types:<br>`1` - Toll-free Call-in & Call-out.<br>`2` - Toll <br>\n`3` - SIP Connected Audio")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ReporttelephoneHandler(cfg),
	}
}
