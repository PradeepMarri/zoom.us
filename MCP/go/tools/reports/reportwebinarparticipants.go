...
func ReportwebinarparticipantsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/report/webinars/%s/participants%s", cfg.BaseURL, webinarId, queryString)
...
func CreateReportwebinarparticipantsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_report_webinars_webinarId_participants",
		mcp.WithDescription("Get webinar participant reports"),
		...
		mcp.WithString("include_fields", mcp.Description("Enter 'registrant_id' as the value for this field if you would like to see the registrant ID attribute included in the response of this API call. A registrant ID is a unique identifier of a [webinar registrant](https://marketplace.zoom.us/docs/api-reference/zoom-api/webinars/webinarregistrants).<br>\n\n")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ReportwebinarparticipantsHandler(cfg),
	}
}
