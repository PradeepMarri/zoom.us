...
func ReportusersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/report/users%s", cfg.BaseURL, queryString)
...
func CreateReportusersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_report_users",
		mcp.WithDescription("Get active/inactive host reports"),
		mcp.WithString("type", mcp.Description("Active or inactive hosts.<br>`active` - Active hosts. <br>`inactive` - Inactive hosts.")),
		...
		mcp.WithNumber("page_number", mcp.Description("The page number of the current page in the returned records.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ReportusersHandler(cfg),
	}
}
