...
func GroupmembersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGroupmembersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_groups_groupId_members",
		mcp.WithDescription("List group members "),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GroupmembersHandler(cfg),
	}
}
