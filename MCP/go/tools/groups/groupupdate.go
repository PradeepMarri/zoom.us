...
func GroupupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/groups/%s%s", cfg.BaseURL, groupId, queryString)
...
func CreateGroupupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_groups_groupId",
		...
		mcp.WithString("name", mcp.Description("Input parameter: Group name. It must be unique to one account and less than 128 characters.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GroupupdateHandler(cfg),
	}
}
