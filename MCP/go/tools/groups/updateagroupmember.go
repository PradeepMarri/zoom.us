...
func UpdateagroupmemberHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/groups/%s/members/%s%s", cfg.BaseURL, groupId, memberId, queryString)
...
func CreateUpdateagroupmemberTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_groups_groupId_members_memberId",
		mcp.WithDescription("Update a group member"),
		mcp.WithString("groupId", mcp.Required(), mcp.Description("Unique Identifier of the group. Can be retrieved by calling [GET /groups](https://marketplace.zoom.us/docs/api-reference/zoom-api/groups/groups) API.<br> To change a member's primary group, provide the groupId of the member's current primary group as the value of this field.<br> To remove a member from one group and move the member to another group, provide the groupId of the group from which the member is to be moved.")),
		mcp.WithString("memberId", mcp.Required(), mcp.Description("Unique Identifier of the group member. Can be retrieved by calling the [List Group Members](https://marketplace.zoom.us/docs/api-reference/zoom-api/groups/groupmembers) API.")),
		mcp.WithString("action", mcp.Required(), mcp.Description("Input parameter: Specify the type of action to be performed:<br><br>\n`move`: Simultaneously remove a member from one group and move the member to a different group.<br>\n`set_primary`: Set a primary group for the user.\n")),
		mcp.WithString("target_group_id", mcp.Description("Input parameter: Can be retrieved by calling [GET /groups](https://marketplace.zoom.us/docs/api-reference/zoom-api/groups/groups) API.<br>\n\nTo move a user, use this field to specify the groupId of the group where the member is to be moved.<br>\n\nTo set a primary group for the user, provide the groupId of the group which is going to be the primary group.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdateagroupmemberHandler(cfg),
	}
}
