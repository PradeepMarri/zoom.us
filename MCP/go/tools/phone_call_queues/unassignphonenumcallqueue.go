...
func UnassignphonenumcallqueueHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/phone/call_queues/%s/phone_numbers/%s%s", cfg.BaseURL, callQueueId, phoneNumberId, queryString)
...
func CreateUnassignphonenumcallqueueTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_phone_call_queues_callQueueId_phone_numbers_phoneNumberId",
		mcp.WithDescription("Unassign a phone number"),
		mcp.WithString("callQueueId", mcp.Required(), mcp.Description("Unique Identifier of the Call Queue. This can be retrieved from the List Call Queues API.")),
		mcp.WithString("phoneNumberId", mcp.Required(), mcp.Description("Unique Identifier of the Phone Number. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UnassignphonenumcallqueueHandler(cfg),
	}
}
