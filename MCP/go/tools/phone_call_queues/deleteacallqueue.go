...
func DeleteacallqueueHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleteacallqueueTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_phone_call_queues_callQueueId",
		mcp.WithDescription("Delete a call queue"),
		mcp.WithString("callQueueId", mcp.Required(), mcp.Description("Unique Identifier of the call queue.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteacallqueueHandler(cfg),
	}
}
