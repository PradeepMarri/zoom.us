...
func ListcallqueuesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateListcallqueuesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_phone_call_queues",
		mcp.WithDescription("List call queues"),
		...
		mcp.WithNumber("page_size", mcp.Description("The number of records returned from a single API call.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListcallqueuesHandler(cfg),
	}
}
