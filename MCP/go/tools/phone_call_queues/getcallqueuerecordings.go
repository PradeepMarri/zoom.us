...
func GetcallqueuerecordingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/phone/call_queues/%s/recordings%s", cfg.BaseURL, callQueueId, queryString)
...
func CreateGetcallqueuerecordingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_phone_call_queues_callQueueId_recordings",
		mcp.WithDescription("Get call queue recordings"),
		...
		mcp.WithString("from", mcp.Description("Start date (within a 6 month range).")),
		mcp.WithString("to", mcp.Description("End date (within a 6 month range).")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetcallqueuerecordingsHandler(cfg),
	}
}
