...
func UpdatephonesettingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/accounts/%s/phone/settings%s", cfg.BaseURL, accountId, queryString)
...
func CreateUpdatephonesettingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_accounts_accountId_phone_settings",
		mcp.WithDescription("Update BYOC settings"),
		mcp.WithString("accountId", mcp.Required(), mcp.Description("Unique identifier of the sub account.")),
		mcp.WithObject("byoc", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdatephonesettingsHandler(cfg),
	}
}
