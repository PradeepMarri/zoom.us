...
func PhoneuserrecordingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/phone/users/%s/recordings%s", cfg.BaseURL, userId, queryString)
...
func CreatePhoneuserrecordingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_phone_users_userId_recordings",
		mcp.WithDescription("Get user's recordings"),
		...
		mcp.WithString("from", mcp.Description("Start date for the query in 'yyyy-mm-dd' format. The date range defined by the \"from\" and \"to\" parameters should only be one month as the response includes only one month worth of recording data. The month defined should fall within the last six months.")),
		mcp.WithString("to", mcp.Required(), mcp.Description("End date.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    PhoneuserrecordingsHandler(cfg),
	}
}
