...
func ListsettingtemplatesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateListsettingtemplatesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_phone_setting_templates",
		mcp.WithDescription("List setting templates"),
		mcp.WithNumber("page_size", mcp.Description("Number of records returns within a single API call.")),
		...
		mcp.WithString("site_id", mcp.Description("Unique identifier of the site. This field is required only if multiple sites have been enabled.  of the site. Required only when multiple sites are enabled. See [Managing multiple sites](https://support.zoom.us/hc/en-us/articles/360020809672-Managing-multiple-sites) for details. If this is not provided, the response lists the account level setting templates.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListsettingtemplatesHandler(cfg),
	}
}
