...
func PhoneusersettingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/phone/users/%s/settings%s", cfg.BaseURL, userId, queryString)
...
func CreatePhoneusersettingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_phone_users_userId_settings",
		mcp.WithDescription("Get user's settings"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    PhoneusersettingsHandler(cfg),
	}
}
