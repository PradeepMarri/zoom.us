...
func PhoneuservoicemailsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/phone/users/%s/voice_mails%s", cfg.BaseURL, userId, queryString)
...
func CreatePhoneuservoicemailsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_phone_users_userId_voice_mails",
		mcp.WithDescription("Get user's voicemails"),
		...
		mcp.WithString("status", mcp.Description("Status of the voice mail")),
		...
		mcp.WithString("from", mcp.Description("Start date for the query in 'yyyy-mm-dd' format. The date range defined by the \"from\" and \"to\" parameters should only be one month as the response includes only one month worth of voicemail data. The month defined should fall within the last six months.")),
		mcp.WithString("to", mcp.Required(), mcp.Description("End date.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    PhoneuservoicemailsHandler(cfg),
	}
}
