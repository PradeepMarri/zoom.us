...
func ListcommonareaphonesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateListcommonareaphonesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_phone_common_area_phones",
		mcp.WithDescription("List common area phones"),
		mcp.WithNumber("page_size", mcp.Description("The total number of records returned from a single API call.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListcommonareaphonesHandler(cfg),
	}
}
