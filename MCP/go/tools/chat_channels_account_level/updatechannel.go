...
func UpdatechannelHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/chat/users/%s/channels/%s%s", cfg.BaseURL, channelId, userId, queryString)
...
func CreateUpdatechannelTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_chat_users_userId_channels_channelId",
		mcp.WithDescription("Update a channel"),
		mcp.WithString("channelId", mcp.Required(), mcp.Description("Channel Id.")),
		mcp.WithString("userId", mcp.Required(), mcp.Description("Unique Identifier of the Zoom user who is the owner of the channel.")),
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: A new name for the channel.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdatechannelHandler(cfg),
	}
}
