...
func GetpsoperationlogsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["category_type"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("category_type=%v", val))
		}
		...
		url := fmt.Sprintf("%s/phone/reports/operationlogs%s", cfg.BaseURL, queryString)
...
func CreateGetpsoperationlogsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_phone_reports_operationlogs",
		mcp.WithDescription("Get operation logs report"),
		mcp.WithString("from", mcp.Description("Start date in 'yyyy-mm-dd' format. The date range defined by the \"from\" and \"to\" parameters should only be one month as the report provides only one month worth of data per API request.")),
		mcp.WithString("to", mcp.Description("End date in 'yyyy-mm-dd' format.")),
		mcp.WithString("category_type", mcp.Description("Filter the response by the category of the action performed. By default, the value of this field is \"all\" and thus, the response will include log of all operations for the defined period.<br><br>To only include response for a specific category type, provide a value for `category_type` from this [table](http://marketplace.zoom.us/docs/phone-operation-categories ).")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetpsoperationlogsHandler(cfg),
	}
}
